...
angular.module('tlContinuousScroll').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('tlContinuousScroll/continuousScroll.html',
    "<div tl-full-height><div tl-when-scrolled=\"scrollCtrl.search()\" tl-has-scroll-content=\"scrollCtrl.isScrolling\" style=\"overflow-x: hidden; height: 100%\"><div style=\"display: table; width: 100%\"><div ng-transclude></div><ng-include src=\"config().noItemTemplate\" ng-if=\"scrollCtrl.loaded && config().items.length === 0\"></ng-include><div class=\"text-center\" ng-if=\"!scrollCtrl.loaded && config().items.length === 0\">{{'_loading' | translate }}...</div></div></div></div>"
  );
